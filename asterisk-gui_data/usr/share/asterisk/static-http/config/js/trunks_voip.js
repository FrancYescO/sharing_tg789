...
var show_Edit_Trunk = function(){
	...
	if( isNewTrunk == true ) {
		...
		ASTGUI.resetTheseFields([ DOM_edit_VOIPTrunk_Type, DOM_edit_VOIPTrunk_Hostname , DOM_edit_VOIPTrunk_Username ,  DOM_edit_VOIPTrunk_Password , 'trunk_obcid' , 'edit_VOIPTrunk_Providername','trunk_fromdomain', 'trunk_fromuser','trunk_authuser','trunk_contact','trunk_insecure' ]);
		...
		ASTGUI.updateFieldToValue( 'trunk_authuser' , getProperty(tinfo, 'authuser') );
                ASTGUI.updateFieldToValue( 'trunk_contact' , getProperty(tinfo, 'contact') );
			...
			if( tmp_authuser ){
				...
				if( parent.sessionData.pbxinfo.trunks[ttype][EDIT_TRUNK].hasOwnProperty('authuser') ){
					...
					x.new_action('delete', EDIT_TRUNK , 'authuser' );
                        ...
                        // test aggiunta campo
                        var tmp_contact = ASTGUI.getFieldValue('trunk_contact');
			if( tmp_contact ){
				x.new_action('update', EDIT_TRUNK , 'contact', tmp_contact );
				parent.sessionData.pbxinfo.trunks[ttype][EDIT_TRUNK]['contact'] = tmp_contact ;
			}else{
				if( parent.sessionData.pbxinfo.trunks[ttype][EDIT_TRUNK].hasOwnProperty('contact') ){
					delete parent.sessionData.pbxinfo.trunks[ttype][EDIT_TRUNK]['contact'] ;
					x.new_action('delete', EDIT_TRUNK , 'contact' );
				}
			}
